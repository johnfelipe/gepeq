...
public class SelectOneMenuRenderer extends InputRenderer {
    ...
    protected void encodeLabel(FacesContext context, SelectOneMenu menu, List<SelectItem> selectItems, Class type) throws IOException {
        ...
        if(isValueBlank(label) || label.equals("&nbsp;"))
            writer.write("&nbsp;");
        else
    ...
    protected void encodePanel(FacesContext context, SelectOneMenu menu, List<SelectItem> selectItems, Class type) throws IOException {
        ...
        //}
        StringBuffer panelClass = new StringBuffer(SelectOneMenu.PANEL_CLASS);
        if (menu.getPanelStyleClass() != null) {
        	panelClass.append(' ');
        	panelClass.append(menu.getPanelStyleClass());
        }
        writer.writeAttribute("class", panelClass.toString(), null);
        
        StringBuffer panelStyle = new StringBuffer();
        if (height != -1) {
        	panelStyle.append("height:");
        	panelStyle.append(height);
        	panelStyle.append("px;");
        }
        if (menu.getPanelStyle() != null) {
        	panelStyle.append(menu.getPanelStyle());
        }
        if (panelStyle.length() > 0) {
        	writer.writeAttribute("style", panelStyle.toString(), null);
        }
        ...
        for(SelectItem selectItem : selectItems) {
            ...
            if(itemValue instanceof String) {
                ...
                for(Column column : columns) {
                    ...
                    // UNED: 04-06-2012 - dballestin - Fix needed to apply styleClass and style attributes of 
                    //                                 <p:column> components to the columns of the generated table
                    ...
                    // UNED: 04-06-2012 - dballestin - END - Fix needed to apply styleClass and style attributes of 
                    //                                 <p:column> components to the columns of the generated table
